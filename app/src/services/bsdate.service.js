...
const BSToAD = (givenDate) => {
  ...
  if (
    ...
    !(dateData[2] > 0 && dateData[2] <= calendarData[dateData[0]][dateData[1] - 1])
  ) {
    if (date) {
      //   if (dateData.length < 3) alert("मान्य ढाँचामा नभएको मिति परिणत हुने छैन");
      //   else if (dateData[0] < 1970)
      //     alert("१९७० साल भन्दा कम मिति परिणत हुने छैन");
      //   else if (dateData[0] > 2099)
      //     alert("२०९९ साल भन्दा बढी मिति परिणत हुने छैन");
      //   else alert("पात्रोमा नभएको मिति परिणत हुने छैन");
    }
...
const findLastDayOfMonthOfAdDate = (givenYear, givenMonth) => {
  ...
  const milliDate = new Date(`${year}-${`0${nextMonth}`.slice(-2)}-01`).getTime() - 24 * 60 * 60 * 1000;
...
const ADToBS = (givenDate) => {
  ...
  if (
    ...
    !(separatedDate[2] > 0 && separatedDate[2] <= findLastDayOfMonthOfAdDate(separatedDate[0], separatedDate[1]))
  ) {
    // if (date) {
    //   if (separatedDate.length < 3) {
    // alert("मान्य ढाँचामा नभएको मिति परिणत हुने छैन");
    //   } else alert("पात्रोमा नभएको मिति परिणत हुने छैन");
    // }
  ...
  if (dateInMillisecond >= -1789948800000 && dateInMillisecond <= 2312582400000) {
    ...
    if (equivalentDay > 102) {
      ...
      while (equivalentDay > 0) {
        ...
        if (equivalentDay <= 0) {
          bsDateObject = findBsDateFromTotalNumberOfDays(year + 1, equivalentDay + calendarData[year + 1][12]);
        } else {
          year++;
        }
      }
      ...
      while (equivalentDay > 0) {
        ...
        if (equivalentDay <= 0) {
          if (equivalentDay === 0) {
            bsDateObject = findBsDateFromTotalNumberOfDays(year - 2, calendarData[year - 2][12]);
          } else {
